...
from models.decode import mot_decode, mot_decode_4, mot_decode_4_new, mot_decode_4_new_pos
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, tlwh_next, score, xs, ys,  temp_feat, vis, buffer_size=30):
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        self.xs, self.ys = new_track.pos
        self.tracklet_len = 1
        self.state = TrackState.Tracked
        self._vis = new_track._vis
        self.is_activated = True
        self.frame_id = frame_id
        if new_id:
            self.track_id = self.next_id()
...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset, frame_rate):
    print(cur_dataset)
    ...
    for idx in range(img_len // 2, img_len + 1):
        ...
        with torch.no_grad():
            ...
            if idx > img_len // 2:
                ...
                if (idx - 1) != img_len // 2:
                    hm_extend = np.zeros((1, 1, inp_height // 4, inp_width // 4))
                    l_extend = np.zeros((1, 1, inp_height // 4, inp_width // 4))
                    vis_extend = np.zeros((1, 1, inp_height // 4, inp_width // 4))
                    alpha = 0.98
                    beta = 2
                    p = 2
                    for track in tracked_stracks_all:
                        if track.state == TrackState.Tracked:
                            xs, ys = track.pos
                            if xs < 0 or xs > inp_width // 4:
                                continue
                            if ys < 0 or ys > inp_height // 4:
                                continue
                            xmin = int(xs) - p
                            ymin = int(ys) - p
                            xmax = int(xs) + p
                            ...
                            xmin = np.clip(xmin, a_min=0, a_max=inp_width // 4)
                            ymin = np.clip(ymin, a_min=0, a_max=inp_height // 4)
                            xmax = np.clip(xmax, a_min=0, a_max=inp_width // 4)
                            ymax = np.clip(ymax, a_min=0, a_max=inp_height // 4)
                            score = np.ones((1, 1, ymax - ymin, xmax - xmin)) * track.score
                            l = np.ones((1, 1, ymax - ymin, xmax - xmin)) * track.tracklet_len
                            vis_t = np.ones((1, 1, ymax - ymin, xmax - xmin)) * track._vis
                            concat_score = np.concatenate([hm_extend[:, :, ymin:ymax, xmin:xmax], score], axis=1)
                            concat_l = np.concatenate([l_extend[:, :, ymin:ymax, xmin:xmax], l], axis=1)
                            concat_vis = np.concatenate([vis_extend[:, :, ymin:ymax, xmin:xmax], vis_t], axis=1)
                            # print(np.max(concat_score, axis=1).shape)
                            hm_extend[:, 0, ymin:ymax, xmin:xmax] = np.max(concat_score, axis=1)
                            l_extend[:, 0, ymin:ymax, xmin:xmax] = np.max(concat_l, axis=1)
                            vis_extend[:, 0, ymin:ymax, xmin:xmax] = np.max(concat_vis, axis=1)
                    hm_extend = torch.from_numpy(hm_extend).cuda().float()
                    l_extend = torch.from_numpy(l_extend).cuda().float()
                    vis_extend = torch.from_numpy(vis_extend).cuda().float()
                    hm_balanced = (hm + beta * hm_extend * l_extend) / (1 + beta * l_extend)
                    hm_result = (1 - alpha) * hm_result + alpha * hm_balanced
                else:
                    hm_result = hm
                dets_t, inds_t, xs_t, ys_t = mot_decode_4_new_pos(hm_result, wh_t, K=128)  # dets的坐标是相对于heatmap
                dets_t1, inds_t1, xs_t1, ys_t1 = mot_decode_4_new_pos(hm_result, wh_t1, K=128)
                ...
                dets = np.concatenate([dets_t, dets_t1, xs_t1, ys_t1], axis=1)
                ...
                if len(dets) > 0:  # det都初始成track
                    '''Detections'''
                    detections = [
                        STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), STrack.tlbr_to_tlwh(tlbrs[5:9]), tlbrs[4], tlbrs[-2], tlbrs[-1], f, vis_f, 30)
                        for
                ...
                #     cv2.putText(imgs, str(det._vis), (x1, y1), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
                #
...
def main(args=None):
    ...
    for seq_num in [2,4,5,9,10,11,13]:
        nf, ta, tc, result_filename = run_each_dataset(parser.model_dir, retinanet, parser.dataset_path, 'train',
                                                       ...
                                                       frame_rate=frame_rates.get(seq_num))
        n_frame += nf
...
def cpu_soft_nms(boxes, id_feat, vis_feat, sigma=0.5, Nt=0.3, threshold=0.001, method=2):
    N = boxes.shape[0]
    pos = 0
    maxscore = 0
    maxpos = 0
    for i in range(N):
        ...
        id_feat[i] = id_feat[maxpos]
        vis_feat[i] = vis_feat[maxpos]
        ...
        while pos < N:
            ...
            if iw > 0:
                ...
                if ih > 0:
                    ...
                    # ov = Diou(np.array([tx1, ty1, tx2, ty2]), np.array([x1, y1, x2, y2]))
                    # ov = box_diou(np.array([tx1, ty1, tx2, ty2]), np.array([x1, y1, x2, y2]), 0.5)
                    # print(ov)
                    if method == 1:  # linear
                        if ov > Nt:
                            weight = 1 - ov
                    ...
                    if boxes[pos, 4] < threshold:
                        ...
                        boxes[pos, 9] = boxes[N - 1, 4]
                        boxes[pos, 10] = boxes[N - 1, 10]
                        boxes[pos, 11] = boxes[N - 1, 11]
                        id_feat[pos] = id_feat[N - 1]
                        vis_feat[pos] = vis_feat[N - 1]
                        N = N - 1
                        pos = pos - 1

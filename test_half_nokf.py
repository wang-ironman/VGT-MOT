    ...
    for idx in range(img_len // 2, img_len + 1):
        ...
        with torch.no_grad():
            ...
            if idx > img_len // 2:
                ...
                # print(wh[:, :, 0:20, 0])
                # print(reg[:, :, 0, 0])
                dets_t, inds_t, xs_t, ys_t = mot_decode_4_new_pos(hm, wh_t, K=128)  # dets的坐标是相对于heatmap
                dets_t1, inds_t1, xs_t1, ys_t1 = mot_decode_4_new_pos(hm, wh_t1, K=128)
                ...
                if len(dets) > 0:  # det都初始成track
                    '''Detections'''
                    detections = [
                        STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), STrack.tlbr_to_tlwh(tlbrs[5:9]), tlbrs[4], tlbrs[-2],
                               tlbrs[-1], f, vis_f, 30)
                        for
                ...
                for it in u_track:
                    ...
                    x, y = track.pos
                    x = np.clip(x, a_min=0, a_max=inp_width//4-1)
                    y = np.clip(y, a_min=0, a_max=inp_height//4-1)
                    predicted_wh = wh_t1[0, :, int(y), int(x)].cpu()
                    # print(predicted_wh)
                    xmin = np.array(x + 0.5 - predicted_wh[0])
                    xmax = np.array(x + 0.5  + predicted_wh[1])
                    ymin = np.array(y + 0.5  - predicted_wh[2])
                    ymax = np.array(y + 0.5  + predicted_wh[3])
                    det = post_process(torch.from_numpy(np.array([[[xmin, ymin, xmax, ymax, 1.0, 0]]])), meta)
                    track.xs = int((xmax + xmin) / 2)
                    track.ys = int((ymax + ymin) / 2)
                    track._tlwh_next = STrack.tlbr_to_tlwh(det.get(1)[0, :4])

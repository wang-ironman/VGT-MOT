    ...
    for idx in range(img_len // 2, img_len + 1):
        ...
        with torch.no_grad():
            ...
            if idx > img_len // 2:
                ...
                #
                # imgs_next = img_origin1
                # if not os.path.exists(os.path.join(model_dir, 'result_half', cur_dataset + "_det_next_KF")):
                #     os.mkdir(os.path.join(model_dir, 'result_half', cur_dataset + "_det_next_KF"))
                ...
                #
                # cv2.imwrite(
                #     os.path.join(model_dir, 'result_half', cur_dataset + "_det_next_KF") + "/" + str(idx) + ".jpg",
                #     imgs_next)
                # continue
                #
                    ...
                    for k in range(trace_len_draw - draw_interval):
                        if (k % draw_interval == 0):
                            ...
                            cv2.line(img, draw_point1, draw_point2, color=color_list[trace_id % len(color_list)],
                                     thickness=2)
                if not os.path.exists(os.path.join(model_dir, 'result_half_line', cur_dataset + "_line")):
                    os.mkdir(os.path.join(model_dir, 'result_half_line', cur_dataset + "_line"))

                cv2.imwrite(
                    os.path.join(model_dir, 'result_half_line', cur_dataset + "_line") + "/" + str(idx) + ".jpg",
                    img)

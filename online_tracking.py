...
def fuse_motion(kf, cost_matrix, tracks, detections, only_position=False, lambda_=0.98):
    ...
    return cost_matrix
def tracking():
    videoName = 'MOT17-02.mp4'
    cap_2 = cv2.VideoCapture(videoName)
    retinanet = create_model('dla_34', {'hm': 1, 'vis': 1,
                                        'wh': 8}, 256)
    
    retinanet = load_model(retinanet, 'exp/no_atten_half_vis_cor_test_4_1/model_final.pt')
    use_gpu = True
    ...
    while True:
        ...
        if ret:
            timer.tic()
            if count % 20 == 0:
                print('Processing frame {}/{} ({:.2f} fps)'.format(videoName, count, 1. / max(1e-5, timer.average_time)))
            with torch.no_grad():
                ...
                cls_feat, reg_feat, vis_feat, id_feat, last_feat = retinanet(img_blob.cuda().float(),
                                                                             last_feat=last_feat)
                ...
                if count > 0:
                    ...
                    if len(dets) > 0:  # det都初始成track
                        '''Detections'''
                        detections = [
                            STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), STrack.tlbr_to_tlwh(tlbrs[5:9]), tlbrs[4], f, vis_f,
                                   30)
                            for
                    ...
                    if (count - 1) == 0:
                        ...
                        for inew in range(len(detections)):
                            ...
                            if track.score < confidence_threshold:
                                continue
                            track.activate(kalman_filter, count, is_Start=True)
                        ...
                        timer.toc()
                        # save results
                        results.append((count, online_tlwhs, online_ids))
                        count = count + 1
                        continue
                    ...
                    for itracked, idet in matches:
                        ...
                        det = detections[idet]
                        if track.state == TrackState.Tracked:
                            track.update(detections[idet], count)
                            activated_starcks.append(track)
                        else:
                            track.re_activate(det, count, new_id=False)
                    ...
                    for itracked, idet in matches:
                        unconfirmed[itracked].update(detections[idet], count)
                    ...
                    for inew in u_detection:
                        ...
                        if track.score < confidence_threshold:
                            continue
                        track.activate(kalman_filter, count, is_Start=False)
                        activated_starcks.append(track)
                    """ Step 5: Update state"""
                    for track in lost_stracks_all:
                        if count - track.end_frame > retention_threshold:
                            track.mark_removed()
                            removed_stracks.append(track)
                    ...
                    tracked_stracks_all, lost_stracks_all = remove_duplicate_stracks(tracked_stracks_all,
                                                                                     lost_stracks_all)
                    ...
                    for j in range(len(online_targets)):
                        ...
                        cv2.rectangle(img, (x1, y1), (x2, y2), color=color_list[trace_id % len(color_list)],
                                      thickness=2)

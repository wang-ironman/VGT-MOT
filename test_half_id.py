...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, tlwh_next, score, temp_feat, buffer_size=30):
...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset, frame_rate, det_path):
    ...
    for idx in range(img_len // 2, img_len + 1):
        ...
        with torch.no_grad():
            ...
            cls_feat, reg_feat, id_feat, last_feat = retinanet(img_blob.cuda().float(), last_feat=last_feat)
            ...
            if idx > img_len // 2:
                ...
                if len(dets) > 0:  # det都初始成track
                    '''Detections'''
                    detections = [
                        STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), STrack.tlbr_to_tlwh(tlbrs[5:9]), tlbrs[4], f, 30)
                        for
                ...
                imgs_next = old_img
                if not os.path.exists(os.path.join(model_dir, 'result_half', cur_dataset + "_det_next")):
                    os.mkdir(os.path.join(model_dir, 'result_half', cur_dataset + "_det_next"))
                ...
                cv2.imwrite(
                    os.path.join(model_dir, 'result_half', cur_dataset + "_det_next") + "/" + str(idx) + ".jpg",
                    imgs_next)
                ...
                # dists = embedding_distance(r_tracked_stracks, detections)

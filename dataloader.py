...
import skimage
from tools.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian
...
class CSVDataset(Dataset):
    ...
    def __init__(self, root_path, train_file, class_list, height=608, width=1088, transform=None):
        """
        Args:
        ...
        self.down_ratio = 4
        self.max_objs = 128
        self.mean = torch.from_numpy(np.array([0.40789654, 0.44719302, 0.47026115],  # BGR
                        dtype=np.float32)).reshape(3, 1, 1)
        self.std = torch.from_numpy(np.array([0.28863828, 0.27408164, 0.27809835],
                       dtype=np.float32)).reshape(3, 1, 1)
        ...
        try:
            ...
            with self._open_for_csv(self.train_file) as file:
                self.image_data = self._read_annotations(csv.reader(file), self.classes)
    ...
    def _get_random_surroud_name_half(self, image_name, max_diff=3, ignore_equal=True, pos_only=False):
        seq_length_dict = {"MOT17-02": 600, "MOT17-04": 1050, "MOT17-05": 837, "MOT17-09": 525, "MOT17-10": 654,
                           "MOT17-11": 900, "MOT17-13": 608}

        suffix_name = image_name.split('/')[-1].split('_')[-1]
        prefix = image_name[0:-len(suffix_name)]
        seq_name = prefix.split('/')[-3]
        cur_index = int(float(suffix_name.split('.')[0]))
        # total_number = seq_length_dict[seq_name]
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                img, annot_t, img_next, annot_t1 = self.load_image_and_anno_pair(idx)
                # print(img)
                # for ann in annot_t:
                #     w = ann[2]
                #     h = ann[3]
                #     if w > h:
                #         print(ann)
                if (annot_t.shape[0] < 1) or (annot_t1.shape[0] < 1):
        ...
        if self.transform:
            annot_t = Variable(torch.from_numpy(annot_t))
            annot_t1 = Variable(torch.from_numpy(annot_t1))
            annot_t = annot_t.reshape(-1, 8)
            annot_t1 = annot_t1.reshape(-1, 8)
    ...
    def load_image_and_anno_pair(self, image_index):
        img_path = self.image_names[image_index]

        img_path_next = self._get_random_surroud_name(img_path, max_diff=3)
        # print(img_path, img_path_next)
        annotation_list = self.image_data[self.image_names[image_index]]
        annotation_list_next = self.image_data[img_path_next]

        height = self.height
        width = self.width
        img = cv2.imread(img_path)  # BGR
        # print(img)
        img_next = cv2.imread(img_path_next)  # (1080, 1920, 3)
        ...
        if self.augment and augment_hsv:
            ...
            fraction = 0.50
            img_hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)  # (1080, 1920, 3)  H [0, 179] S [0, 255] V [0, 255]
            img_hsv_next = cv2.cvtColor(img_next, cv2.COLOR_BGR2HSV)

            S = img_hsv[:, :, 1].astype(np.float32)
            V = img_hsv[:, :, 2].astype(np.float32)
            S_next = img_hsv_next[:, :, 1].astype(np.float32)
            V_next = img_hsv_next[:, :, 2].astype(np.float32)
            ...
            if a > 1:
                ...
                np.clip(S_next, a_min=0, a_max=255, out=S_next)

            a = (random.random() * 2 - 1) * fraction + 1  # random.random() -> float (0, 1)

            V *= a
            V_next *= a
            if a > 1:
                np.clip(V, a_min=0, a_max=255, out=V)
                np.clip(V_next, a_min=0, a_max=255, out=V_next)

            img_hsv[:, :, 1] = S.astype(np.uint8)
            img_hsv[:, :, 2] = V.astype(np.uint8)
            img_hsv_next[:, :, 1] = S_next.astype(np.uint8)
            img_hsv_next[:, :, 2] = V_next.astype(np.uint8)

            cv2.cvtColor(img_hsv, cv2.COLOR_HSV2BGR, dst=img)
            cv2.cvtColor(img_hsv_next, cv2.COLOR_HSV2BGR, dst=img_next)

        h, w, _ = img.shape
        img, img_next, ratio, padw, padh = letterbox(img, img_next, height=height, width=width)
        # print(img.shape)
        # Load labels

        if len(annotation_list) != 0:
            # print(np.array(annotation_list))
            labels0 = np.array(annotation_list).reshape(-1, 8)
            # [ctr_x, ctr_y, w, h,  class_idx, obj_id]
        ...
        if len(annotation_list_next) != 0:
            # print(np.array(annotation_list))
            labels0_next = np.array(annotation_list_next).reshape(-1, 8)
            # [ctr_x, ctr_y, w, h,  class_idx, obj_id]

            # Normalized xywh to pixel xyxy format
            labels0_next = labels0_next.astype(np.float)

            labels_next = labels0_next.copy()

            labels_next[:, 0] = ratio * w * (labels0_next[:, 0] - labels0_next[:, 2] / 2) + padw
            labels_next[:, 1] = ratio * h * (labels0_next[:, 1] - labels0_next[:, 3] / 2) + padh
            labels_next[:, 2] = ratio * w * (labels0_next[:, 0] + labels0_next[:, 2] / 2) + padw
            labels_next[:, 3] = ratio * h * (labels0_next[:, 1] + labels0_next[:, 3] / 2) + padh

        else:
            labels_next = np.array([])

        if self.augment:
            img, labels, img_next, labels_next, M = random_affine(img, img_next, labels, labels_next, degrees=(-5, 5),
                                                                  translate=(0.10, 0.10), scale=(0.50, 1.20))

        # for i, cor in enumerate(labels):
        #     if cor[-1] < 0:
        #         print(cor)
        #     # print(cor)
        #     labels[i, -1] = compute_vis(cor)
        ...
        #         print(cor)
        if nL_next > 0:
            labels_next[:, 0:4] = xyxy2xywh(labels_next[:, 0:4].copy())  # / height
            labels_next[:, 0] /= width
            labels_next[:, 1] /= height
            labels_next[:, 2] /= width
            labels_next[:, 3] /= height
        if self.augment:
            # random left-right flip
            lr_flip = True
            if lr_flip & (random.random() > 0.5):
                img = np.fliplr(img)
                img_next = np.fliplr(img_next)
                if nL > 0:
                    labels[:, 0] = 1 - labels[:, 0]
                    labels_next[:, 0] = 1 - labels_next[:, 0]

        img = np.ascontiguousarray(img)  # no BGR to RGB
        img_next = np.ascontiguousarray(img_next)
        if self.transform is not None:
            img = self.transform(img)  # toTensor auto divide 255
            img_next = self.transform(img_next)
        img = (img - self.mean) / self.std
        img_next = (img_next - self.mean) / self.std
        return img, labels, img_next, labels_next


    def load_annotations(self, image_index):
        # get ground truth annotations
        annotation_list = self.image_data[self.image_names[image_index]]
        annotations = np.zeros((0, ))
    ...
    def _read_annotations(self, csv_reader, classes):
        ...
        for line, row in enumerate(csv_reader):
            ...
            # print(row)
            try:
                img_file, obj_id, ctr_x, ctr_y, w, h, vis, class_name = row[:8]
                
            except ValueError:
                raise_from(ValueError(
                    'line {}: format should be \'img_file,obj_id,x1,y1,x2,y2,class_name,vis\' or \'img_file,,,,,\''.format(
                        line)), None)
            ...
            class_idx = self.name_to_label(class_name)
            # If a row contains only an image path, it's an image without annotations.
            if (ctr_x, ctr_y, w, h, vis, class_name) == ('', '', '', '', '', ''):
                continue
            ...
            result[img_file].append([ctr_x, ctr_y, w, h, class_idx, obj_id, vis, 0])
        return result
...
def collater(data):
    ...
    for i in range(batch_size):
        ...
        img_next = img_next.permute(1, 2, 0)
    ...
    if max_num_annots > 0:
        annot_padded_next = torch.ones((len(annots_next), max_num_annots, 8)) * -1
...
class Normalizer(object):
    ...
    def __call__(self, sample):
        ...
        return {'img': torch.from_numpy((image.astype(np.float32) / 255.0 - self.mean) / self.std),
                'annot': torch.from_numpy(annots),
                'img_next': torch.from_numpy((image_next.astype(np.float32) / 255.0 - self.mean) / self.std),
                'annot_next': torch.from_numpy(annots_next)}
...
def letterbox(img, img_next, height=608, width=608,
    ...
    img_next = cv2.resize(img_next, new_shape, interpolation=cv2.INTER_AREA)
    img = cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # padded rectangular
    img_next = cv2.copyMakeBorder(img_next, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)
    return img, img_next, ratio, dw, dh

def letterbox_test(img, height=608, width=1088,
              color=(127.5, 127.5, 127.5)):  # resize a rectangular image to a padded rectangular
    shape = img.shape[:2]  # shape = [height, width]
    ratio = min(float(height) / shape[0], float(width) / shape[1])
    new_shape = (round(shape[1] * ratio), round(shape[0] * ratio))  # new_shape = [width, height]
...
def compute_vis(cor, height=608, width=1088):
    xmin = cor[0]
    ymin = cor[1]
    xmax = cor[2]
    ymax = cor[3]
    ...
    if flag:
        ...
        vis = min(cor[-2], area_new/area)
        weight = 1.0  # enhance effect of border point detection
        # if vis < 0:
        #     print(cor[-1], area_new/area)
        #
        #     print(xmin, ymin, xmax, ymax)
        #     print(xmin_new, ymin_new, xmax_new, ymax_new)
        #     print('------')
        ...
        if cor[-1] == 1.0:
            ...
            weight = cor[-2]
    return np.array([vis, weight])

def random_affine(img, img_next, targets=None, targets_next=None, degrees=(-10, 10), translate=(.1, .1),
    ...
    imw_next = cv2.warpPerspective(img_next, M, dsize=(width, height), flags=cv2.INTER_LINEAR,
    ...
    if targets is not None:
        if len(targets) > 0:
            ...
            targets = targets[i]
            targets[:, 0:4] = xy[i]
            for i, cor in enumerate(targets):
                # print()
                targets[i, -2:] = compute_vis(cor)
                # print(compute_vis(cor), targets[i, -2:])
            flag = targets[:, -2] > 0
            targets = targets[flag]
    if targets_next is not None:
        if len(targets_next) > 0:
            n = targets_next.shape[0]
            points = targets_next[:, 0:4].copy()
            area0 = (points[:, 2] - points[:, 0]) * (points[:, 3] - points[:, 1])
            for cor in targets_next:
                xmin = cor[0]
                ymin = cor[1]
                xmax = cor[2]
                ymax = cor[3]
            ...
            targets_next = targets_next[i]
            targets_next[:, 0:4] = xy[i]
            for i, cor in enumerate(targets_next):
                targets_next[i, -2:] = compute_vis(cor)
            flag = targets_next[:, -2] > 0
            targets_next = targets_next[flag]

        return imw, targets, imw_next, targets_next, M
    else:
        return imw, imw_next
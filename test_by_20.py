...
from models.decode import mot_decode, mot_decode_4, mot_decode_4_new
...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, tlwh_next, score, temp_feat, vis, buffer_size=30):
...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset, frame_rate, det_path):
    ...
    max_draw_len = 100
    draw_interval = 5
    if cur_dataset == 'MOT20-04':
        img_width = 1545
        img_height = 1080
    elif cur_dataset == 'MOT20-06':
        img_width = 1920
        img_height = 734
    elif cur_dataset == 'MOT20-07':
        img_width = 1920
        img_height = 1080
    else:
        img_width = 1920
    ...
    for idx in range(0, img_len + 1):
        ...
        with torch.no_grad():
            ...
            if idx > 0:
                ...
                # print(wh[:, :, 0:20, 0])
                # print(reg[:, :, 0, 0])
                dets_t, inds_t = mot_decode_4_new(hm, wh_t, K=512)  # dets的坐标是相对于heatmap
                dets_t1, inds_t1 = mot_decode_4_new(hm, wh_t1, K=512)
                ...
                dets_t = merge_outputs([dets_t], K=512)[1]
                dets_t1 = post_process(dets_t1, meta)  # 转换成针对原图的坐标
                dets_t1 = merge_outputs([dets_t1], K=512)[1]
                dets = np.concatenate([dets_t, dets_t1], axis=1)

                remain_inds = dets[:, 4] > track_threshold
                remain_inds_02 = dets[:, 4] < track_threshold

                det_04 = dets[remain_inds_02]
                id_feature_04 = id_feature[remain_inds_02]
                vis_feature_04 = vis_feature[remain_inds_02]
                remain_inds_02 = det_04[:, 4] > 0.2
                det_02 = det_04[remain_inds_02]
                dets = dets[remain_inds]




                np.clip(dets[:, 0], 0, width, out=dets[:, 0])
                np.clip(dets[:, 1], 0, height, out=dets[:, 1])
                np.clip(dets[:, 2], 0, width, out=dets[:, 2])
                np.clip(dets[:, 3], 0, height, out=dets[:, 3])

                np.clip(dets[:, 5], 0, width, out=dets[:, 5])
                np.clip(dets[:, 6], 0, height, out=dets[:, 6])
                np.clip(dets[:, 7], 0, width, out=dets[:, 7])
                np.clip(dets[:, 8], 0, height, out=dets[:, 8])

                np.clip(det_02[:, 0], 0, width, out=det_02[:, 0])
                np.clip(det_02[:, 1], 0, height, out=det_02[:, 1])
                np.clip(det_02[:, 2], 0, width, out=det_02[:, 2])
                np.clip(det_02[:, 3], 0, height, out=det_02[:, 3])

                np.clip(det_02[:, 5], 0, width, out=det_02[:, 5])
                np.clip(det_02[:, 6], 0, height, out=det_02[:, 6])
                np.clip(det_02[:, 7], 0, width, out=det_02[:, 7])
                np.clip(det_02[:, 8], 0, height, out=det_02[:, 8])
                for i in range(dets.shape[0]):
                ...
                id_feature_02 = id_feature_04[remain_inds_02]
                vis_feature_02 = vis_feature_04[remain_inds_02]
                id_feature = id_feature[remain_inds]
                vis_feature = vis_feature[remain_inds]

                keep = nms(dets[:, :5], thresh=0.4)
                dets = dets[keep, :]
                id_feature = id_feature[keep, :]
                vis_feature = vis_feature[keep, :]

                keep = nms(det_02[:, :5], thresh=0.4)
                det_02 = det_02[keep, :]
                id_feature_02 = id_feature_02[keep, :]
                vis_feature_02 = vis_feature_02[keep, :]

                if len(dets) > 0:  # det都初始成track
                    '''Detections'''
                    detections = [
                        STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), STrack.tlbr_to_tlwh(tlbrs[5:9]), tlbrs[4], f, vis_f, 30)
                        for
                        (tlbrs, f, vis_f) in zip(dets[:, :], id_feature, vis_feature)]
                else:
                    detections = []
                if len(det_02) > 0:  # det都初始成track
                    '''Detections'''
                    detections_02 = [
                        STrack(STrack.tlbr_to_tlwh(tlbrs[:4]), STrack.tlbr_to_tlwh(tlbrs[5:9]), tlbrs[4], f, vis_f, 30)
                        for
                        (tlbrs, f, vis_f) in zip(det_02[:, :], id_feature_02, vis_feature_02)]
                else:
                    detections_02 = []
                # imgs = old_img
                # imgs_next = img_origin1
                # if not os.path.exists(os.path.join(model_dir, 'result', cur_dataset + "_det")):
                #     os.mkdir(os.path.join(model_dir, 'result', cur_dataset + "_det"))
                ...
                #     # cv2.rectangle(imgs, (x1_n, y1_n), (x2_n, y2_n), color=(0, 255, 0), thickness=2)
                #     # cv2.putText(imgs, str(det.score) + ',' + str(det._vis), (x1, y1), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
                #
                # cv2.imwrite(os.path.join(model_dir, 'result', cur_dataset + "_det") + "/" + str(idx) + ".jpg", imgs)
                # continue
                ...
                if (idx - 1) == 0:
                    ...
                    if not os.path.exists(os.path.join(model_dir, 'result', cur_dataset + '30')):
                        os.mkdir(os.path.join(model_dir, 'result', cur_dataset + '30'))
                    # print(os.path.join(model_dir, 'result', cur_dataset, '{:05d}.jpg'.format(idx)))
                    # print(online_im.shape)
                    cv2.imwrite(os.path.join(model_dir, 'result', cur_dataset + '30', '{:05d}.jpg'.format(idx)),
                                online_im)
                ...
                # imgs_next = img_origin1
                # if not os.path.exists(os.path.join(model_dir, 'result', cur_dataset + "_det_next_KF")):
                #     os.mkdir(os.path.join(model_dir, 'result', cur_dataset + "_det_next_KF"))
                # for i, det in enumerate(strack_pool):
                #     if det.state != TrackState.Tracked:
                #         continue
                #     tlwh = det.tlwh
                #     # vis = vis_feature[i]
                #     # print(tlwh)
                ...
                #
                # cv2.imwrite(
                #     os.path.join(model_dir, 'result', cur_dataset + "_det_next_KF") + "/" + str(idx) + ".jpg",
                #     imgs_next)
                # continue
                ...
                for itracked, idet in matches:
                    ...
                    #     track.re_activate(det, idx, new_id=False)
                    #     refind_stracks.append(track)
                r_tracked_stracks = [r_tracked_stracks[i] for i in u_track]  # why
                dists = iou_distance(r_tracked_stracks, detections_02)
                matches, u_track, u_detection_02 = linear_assignment(dists, thresh=0.5)

                for itracked, idet in matches:
                    track = r_tracked_stracks[itracked]
                    det = detections_02[idet]
                    if track.state == TrackState.Tracked:
                        # print("22")
                        track.update(det, idx)
                        activated_starcks.append(track)
                ...
                removed_stracks_all.extend(removed_stracks)
                ...
                # print(os.path.join(model_dir, 'result', cur_dataset, '{:05d}.jpg'.format(idx)))
                # print(online_im.shape)
                cv2.imwrite(os.path.join(model_dir, 'result', cur_dataset + '30', '{:05d}.jpg'.format(idx)), online_im)
                videoWriter.write(online_im)
                ...
                cv2.waitKey(0)
        # save results
    write_results(os.path.join(model_dir, 'result', cur_dataset + '.txt'), results, 'mot')
    videoWriter1.release()
    videoWriter.release()
    return img_len, timer.average_time, timer.calls, os.path.join(model_dir, 'result', cur_dataset + '.txt')
...
def run_from_train(model_dir, root_path):
    if not os.path.exists(os.path.join(model_dir, 'result')):
        os.makedirs(os.path.join(model_dir, 'result'))
    if not os.path.exists(os.path.join(model_dir, 'result_line')):
        os.makedirs(os.path.join(model_dir, 'result_line'))
...
def main(args=None):
    ...
    parser.add_argument('--dataset_path', default='/home/neuiva2/liweixi/data/MOT20/', type=str,
    ...
    if not os.path.exists(os.path.join(parser.model_dir, 'result')):
        os.makedirs(os.path.join(parser.model_dir, 'result'))
    if not os.path.exists(os.path.join(parser.model_dir, 'result_line')):
        os.makedirs(os.path.join(parser.model_dir, 'result_line'))
    ...
    frame_rates = {1: 30, 3: 30, 6: 14, 7: 30, 8: 30, 12: 30, 14: 25}
    det_file_path = os.path.join(parser.model_dir, "MOT_det_2.txt")
    f = open(det_file_path, 'a+')
    for seq_num in [6,8]:
        nf, ta, tc, result_filename = run_each_dataset(parser.model_dir, retinanet, parser.dataset_path, 'test',
                                                       'MOT20-{:02d}'.format(seq_num),
                                                       frame_rate=25, det_path=f)
        n_frame += nf
        ...
        #
        # output_dir = os.path.join(parser.model_dir, 'result', seq_name)
...
def merge_outputs(detections, K=128):
    ...
    scores = np.hstack(
        [result[j][:, 4] for j in range(1, 1 + 1)])
    if len(scores) > K:
        kth = len(scores) - K
...
def gious(atlbrs, btlbrs):
    ...
    #     np.ascontiguousarray(btlbrs, dtype=np.float)
    # )
    a = np.ascontiguousarray(atlbrs, dtype=np.float)
    b = np.ascontiguousarray(btlbrs, dtype=np.float)
...
def DIoU(rect1, rect2):
    """
    calculate the diou(distance intersection over union) of two boxes.
    param rect1: box1 coordinate [xmin, ymin, xmax, ymax]
    param rect2: box2 coordinate [xmin, ymin, xmax, ymax]
    return diou: diou of box1 and box2
    """
    ...
    # 计算包含两个矩形的矩形框的对角线欧式距离
    diag_distance = (np.max([xmax1, xmax2]) - np.min([xmin1, xmin2])) ** 2 + (
            np.max([ymax1, ymax2]) - np.min([ymin1, ymin2])) ** 2

    # 计算交并比
    diou = inter_area / union_area - center_distance / diag_distance

    return diou


def GIoU(rect1, rect2):
    """
    calculate the giou(generalized intersection over union) of two boxes.
    param rect1: box1 coordinate [xmin, ymin, xmax, ymax]
    param rect2: box2 coordinate [xmin, ymin, xmax, ymax]
    return giou: iou of box1 and box2
    """
    xmin1, ymin1, xmax1, ymax1 = rect1
    xmin2, ymin2, xmax2, ymax2 = rect2
    ...
    # 计算包含两个矩形的矩形框的面积
    area_c = (np.max([xmax1, xmax2]) - np.min([xmin1, xmin2])) * (np.max([ymax1, ymax2]) - np.min([ymin1, ymin2]))
    ...
    # 计算交并比
    giou = inter_area / union_area - (area_c - union_area) / area_c
...
def nms(dets, thresh):
    ...
    while order.size > 0:
        ...
        # ovr = Diou(np.array([x1[i], y1[i], x2[i], y2[i]]),
        #            np.row_stack([x1[order[1:]], y1[order[1:]], x2[order[1:]], y2[order[1:]]]))
        # ovr = bbox_iou(np.array([x1[i], y1[i], x2[i], y2[i]]), np.row_stack([x1[order[1:]], y1[order[1:]], x2[order[1:]], y2[order[1:]]]), GIoU=True)

    ...
    for idx in range(img_len // 2, img_len + 1):
        ...
        with torch.no_grad():
            ...
            if idx > img_len // 2:
                ...
                detections_05 = [t for t in detections if t._vis >= 0.5]
                detections_00 = [t for t in detections if t._vis < 0.5]

                # STrack.multi_predict(strack_pool_lost)
                # dists = embedding_distance(strack_pool, detections)
                dists = mix_distance(strack_pool, detections_05)
                # dists = iou_distance(strack_pool, detections)
                # dists = gate_cost_matrix(self.kalman_filter, dists, strack_pool, detections)
                dists = fuse_motion(kalman_filter, dists, strack_pool, detections_00)
                matches, u_track, u_detection = linear_assignment(dists, thresh=0.7)
                for itracked, idet in matches:
                    track = strack_pool[itracked]
                    det = detections_00[idet]
                    if track.state == TrackState.Tracked:
                        track.update(detections_05[idet], idx)
                ...
                detections = [detections_00[i] for i in u_detection]
                detections = detections + detections_05

                r_tracked_stracks = [strack_pool[i] for i in u_track]  # why
                dists = mix_distance(r_tracked_stracks, detections)
